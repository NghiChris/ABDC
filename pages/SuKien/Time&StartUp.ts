...
export class TimeAndStartUp {
  ...
  constructor(page: Page) {
    this.page = page;
    // this.bannerEvent = page.locator("text=/sự kiện công nghệ lớn nhất 2021/i");
  ...
  async clickThamGia() {
    ...
    // Nếu nút này không điều hướng, kiểm tra nó click được (không lỗi)
  ...
  async verifyStartupImageVisible() {
    ...
    const isLoaded = await this.startupImage.evaluate(
      (img: HTMLImageElement) => img.complete && img.naturalWidth > 0);
    expect(isLoaded).toBeTruthy();
    console.log("✅ Ảnh minh họa Startup hiển thị thành công.");

    // const src = await this.startupImage.getAttribute("src");
    // expect(src).toContain("it.ef68b551.png");
  }
}
